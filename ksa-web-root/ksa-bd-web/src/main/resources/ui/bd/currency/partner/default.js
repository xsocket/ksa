$(function(){
    ...
    // 初始化客户选择框
    $('#partner').combobox({ url: ksa.buildUrl( "/data/combo", "bd-partner-all" ) }); 
    
    // 初始化汇率表
    var $grid = $('#data-grid').datagrid({
        url : ksa.buildUrl( "/data/grid/currency", "partner", { "partnerId" : partnerId } ),
    ...
    function save() {
        ...
        $.each( rates, function(i, rate) {
            ...
            $.ajax({
                ...
                data: { "id": rate.id, "rate": rate.rate, "currency.id": rate.currency.id, "partner.id": partnerId },

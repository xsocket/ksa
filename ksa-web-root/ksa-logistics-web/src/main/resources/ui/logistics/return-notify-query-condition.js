var BOOKINGNOTE_QUERY_CONDITION = [ 
     { title:" 退单", name:"STATE", init: function( target, condition ) {
         $(target).append("<label></label>");
        var input = $("<select name='" + condition.name + "' />").width( $(target).width() - 50 );
        input.append("<option value='268435456'>已退单</option>");
        input.append("<option value='-2'>未退单</option>");
        $(target).append(input);
        input.combobox( {multiple:true,editable:false} );
     } },  
     { title:"托单编号", name:"CODE", type:"text" },
     { title:"退单号", name:"RETURN_CODE", type:"text" },
     { title:"退单日期", name:"RETURN_DATE", type:"date" },
     { title:"税单号", name:"TAX_CODE", type:"text" },
     { title:"收单日期", name:"TAX_DATE1", type:"date" },
     { title:"寄出日期", name:"TAX_DATE2", type:"date" },
     { title:"快件号", name:"EXPRESS_CODE", type:"text" },
     ...
     { title:"业务细分类型", name:"TYPE_SUB", init: function( target, condition ){
         $(target).append("<label></label>");
         var input = $("<select name='" + condition.name + "' />").width( $(target).width() - 50 );
         input.append("<option value='FCL'>FCL</option>");
         input.append("<option value='LCL'>LCL</option>");
         $(target).append(input);
         input.combobox( {multiple:true} );
     } },
     ...
     { title:"销售担当", name:"SALER_ID", type:"combo", option:{url:ksa.buildUrl( "/data/combo", "security-user-all" ),codeField : "id" } },

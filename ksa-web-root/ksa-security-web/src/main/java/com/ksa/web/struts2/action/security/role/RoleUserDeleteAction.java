 ...
 * @author 麻文强
 */
public class RoleUserDeleteAction extends ClearRoleCacheAction implements JsonAction {

    private static final long serialVersionUID = -593371190232164690L;
    ...
    protected String doExecute() throws Exception {
        if( userIds != null && userIds.length > 0 ) {
            ...
            this.result = new JsonResult( SUCCESS, "成功为角色删除用户。", this.role );
        }
        else {
            this.result = new JsonResult( SUCCESS, "没有指定需要删除的用户。");
        }
        return SUCCESS;
    }

if ($.fn.pagination){
	...
	$.fn.pagination.defaults.afterPageText = '页，共 <b>{pages}</b> 页';
	$.fn.pagination.defaults.displayMsg = '显示 <b>{from}</b> 到 <b>{to}</b>，共 <b>{total}</b> 条记录';
}
if( $.fn.panel ) {
...
if ($.fn.datagrid){
	...
	// CUSTOM : 空数据提示
	$.fn.datagrid.defaults.loadEmptyMsg = '<span class="label label-warning">注意</span> 没有获取到任何数据';
}
...
if ($.fn.validatebox){
	...
	$.fn.validatebox.defaults.rules.length.message = '输入内容长度必须介于 <b>{0}</b> 和 <b>{1}</b> 之间';

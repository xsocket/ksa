...
<#-- 资金状态为: 未删除，未审核中，未审核，可以进行编辑 -->
<#assign readonly = stack.findValue("${stateClass}@isDeleted(${state})") 
	|| stack.findValue("${stateClass}@isChecked(${state}, ${direction}, ${nature})") 
	|| stack.findValue("${stateClass}@isChecking(${state}, ${direction}, ${nature})") >
...
<body>
<form id="dialog_container" method="POST" action="<@s.url namespace="/dialog/finance/charge/single" action="save" />">
...
<div class="easyui-layout" data-options="fit:true">
	<div data-options="region:'north'" style="height:156px;padding:5px">
		<div class="form-inline" style="float:left;">
			...
			<div class="control-group">
				<label class="control-label">记账月份：</label>
				<div class="controls">
			...
			<div class="control-group">
				<label class="control-label">提单编号：</label>
				<div class="controls">
					<input type="text" value="${model.mawb!}" />
				</div>
			</div>
			<div class="control-group">
				<label class="control-label">船名/航次：</label>
				<div class="controls">
			...
			<div class="control-group">
				...
				<div class="controls input-append" style="margin-left:0;">
			    	<input type="text" style="text-align:right;" value="${model.weight!}" />
		...
		<div style="float:right; height:137px; width:320px;">
			<table id="profit_datagrid">
			</table>
		</div><#-- end 利润统计 -->
	</div><#-- end north  -->
	<div class="charge-container" data-options="region:'center',border:false">
		<table id="charge_datagrid" title="费用明细列表"></table>
	</div><#-- end center -->
</div>
...
<#elseif stack.findValue("${stateClass}@isChecked(${state}, ${direction}, ${nature})")>
	<span class="title">状态：<b style="color:#51A351;">审核通过</b></span>
<#elseif stack.findValue("${stateClass}@isChecking(${state}, ${direction}, ${nature})")>
...
<#elseif stack.findValue("${stateClass}@isEntering(${state}, ${direction}, ${nature})")>
...
<#else>
	...
	<button id="dialog_download" class="btn btn-success"><i class="icon-download-alt icon-white"></i> 导出面单</button>
...
<div id="charge_window" style="display:none;">
	<form id="charge_form" class="form-inline" style="padding:10px;margin:0;">
		<div class="control-group">
		...
		<div class="control-group">
			...
			<div class="controls">
		    	<input id="charge_type" type="text" name="type" data-options="required:true,validType:'length[0,200]'" />
			</div>
		</div>
		<div class="control-group">
			<label class="control-label">费用类型：</label>
			<div class="controls">
		    	<input type="text" readonly="readonly" value="<#if direction==1>收入<#else>支出</#if><#if nature==-1>（境外）</#if>" />
			</div>
		</div>
		<br />
		<div class="control-group">
		...
		<div class="control-group">
			...
			<div class="controls">
		    	<input id="price" type="text" name="price" style="text-align:right;" class="easyui-numberbox" data-options="min:0,precision:3"/>
			</div>
		</div>
		<div class="control-group">
			<label class="control-label">数量：</label>
			<div class="controls">
		    	<input id="quantity" type="text" name="quantity" style="text-align:right;" class="easyui-numberbox" data-options="min:0" />
			</div>
		</div>
		<br />
		<div class="control-group">

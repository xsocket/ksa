var ACCOUNT_QUERY_CONDITION = [
    { title:"状态", name:"ACCOUNT_STATE", init: function( target, condition ) {
        $(target).append("<label></label>");
        var input = $("<select name='" + condition.name + "' />").width( $(target).width() - 50 );
        input.append("<option value='0'>录入中</option>");
        input.append("<option value='1'>审核中</option>");
        input.append("<option value='2'>开票中</option>");
        input.append("<option value='8'>" + ( DIRECTION == 1 ? "收款中" : "付款中" ) + "</option>");
        input.append("<option value='32'>结算完毕</option>");
        $(target).append(input);
        input.combobox( {multiple:true,editable:false} );
    } },              
    { title:"编号", name:"ACCOUNT_CODE", type:"text" },
    { title:"结算对象", name:"ACCOUNT_TARGET", type:"combo", option:{url:ksa.buildUrl( "/data/combo", "bd-partner-all" )} },
    ...
    { title:"创建人", name:"ACCOUNT_INPUTOR", type:"combo", option:{url:ksa.buildUrl( "/data/combo", "security-user-all" ),codeField : "id", multiple:true,editable:false }  },

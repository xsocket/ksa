var PROFIT_QUERY_CONDITION = [
    { title:"记账月份", name:"CHARGE_DATE", type:"dateMonth" },
    { title:"费用状态", name:"STATE", init: function( target, condition ){
            ...
            input.append("<option value='1'>费用录入中</option>");
            input.append("<option value='2'>费用审核中</option>");
            input.append("<option value='8'>审核通过</option>");
            input.append("<option value='-'>|-- 暂未开单</option>");
            input.append("<option value='-0'>|-- 账单录入中</option>");
            input.append("<option value='-1'>|-- 账单审核中</option>");
